...
**Oct 16, 2025 update: 1. ICAR version of the model is provided. It could be marginally faster for general spatial networks, such as k-NN or Delauney network. 2. Horseshoe models now support general networks as well, please click the "README.md" file to view the documentation properly.**
...
```{r loading the coords, out.width = "30%"}
...
```{r construct kernel matrix, out.width = "30%"}
...
```{r MCMC diagnostic plots, echo = T, results = T,  out.width = "50%",  out.height = "70%"}
set.seed(2025)
...
Display the expression of *y1* and *y2* across the spatial locations (top row). The estimated slope is shown in a *z*-score type format, i.e., scaled by mean and SD, in the bottom row. In the bottom right sub-figure, locations where the local test produced a *p*-value > 0.9 are greyed.

```{r p-value visualization,  out.width="60%"}
...
We are fitting the NB model this time with an ICAR prior (which.prior = "ICAR"). The rest of the arguments are the same as before, except that *G* now denotes a mutual kNN network (with k = 3).


```{r fit SpaceBF NB model with ICAR, echo = T, results = T, out.width="60%"}

# construct a mutual 3-NN graph
k <- 3
nn <- FNN::get.knn(coords[, 1:2], k = k)
...
w_knn <- igraph::as_adjacency_matrix(g, type = "both", attr = NULL, sparse = TRUE)  # 3-NN adjacency matrix
plot(coords[, 1:2], asp = 1, pch = 16, cex = 0.6, col = "grey30",
     xlab = "x", ylab = "y")
plot(w_knn, coords, add = TRUE, col = "steelblue", lwd = 1.2)
title("kNN neighbor graph")


# ICAR model fitting
MM_ICAR <- SpaceBF(y1, y2, X = NULL, G = w_knn, which.model = "NB", which.prior = "ICAR", nIter = 5000) 
...
We compare the local estimates by the HS and ICAR models using the function "plot_local_comparisons". The function takes as input the local results from both models, and produces a scatter plot of the local estimates and p-values.


```{r compare HS vs. ICAR, echo = T, results = T, out.width="60%"}
...
We are fitting the NB model with the spatial horseshoe prior coupled with the kNN network instead of the MST.


```{r fit SpaceBF NB model with horseshoe on Delaunay, echo = T, results = T, out.width="60%"}
MM_HS_kNN <- SpaceBF(y1, y2, X = NULL, G = w_knn, which.model = "NB", which.prior = "HS", nIter = 5000) 
...
```{r compare HS MST vs. HS Delaunay vs. ICAR, echo = T, results = T, out.width="60%"}
