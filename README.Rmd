...
We are fitting the NB model this time with an ICAR prior (which.prior = "ICAR"). The rest of the arguments are the same as before, except that *G* now denotes the Delauney network constructed using the MERINGUE package.
...
We are fitting the NB model with the spatial horseshoe prior coupled with the Delaunay network instead of the MST.


```{r fit SpaceBF NB model with horseshoe on Delaunay, echo = T, results = T, out.width="60%"}
MM_HS_Del <- SpaceBF(y1, y2, X = NULL, G = w, which.model = "NB", which.prior = "HS", nIter = 5000) 

plot_estimates(MM_HS_Del$Beta[, -c(1:N)], y1, y2, coords[, 1:2], which.model = "NB")
...
plot_local_comparisons(L_results_ICAR, L_results_Del)
