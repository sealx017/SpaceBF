...
data("SpaceBF_cSCC")
coords_data_cSCC <- SpaceBF_cSCC$coords_data_cSCC
gene_data_cSCC <- SpaceBF_cSCC$gene_data_cSCC 
Keratin_pairs <- SpaceBF_cSCC$Keratin_pairs
...
kNN_obj <- SpaceBF::kNN_construction(coords_data_cSCC, k = 4)
...
parallel::clusterExport(cl, varlist = c("gene_data_cSCC", "w_knn",
...
for(l in 1:nrow(Keratin_pairs)){
  
  location_data_with_beta <- as.data.frame(cbind(coords_data_cSCC, 
                             log(gene_data_cSCC[ , Keratin_pairs[l, 2]] + 1)))
  location_data_with_beta$y2 <- log(gene_data_cSCC[ , Keratin_pairs[l, 1]] + 1)
  ...
  p1 <- ggplot(data =  p.sf) +
    ...
    ggtitle(paste0("Gene: ", Keratin_pairs[l, 2]))+
    theme(axis.text = element_blank(), axis.ticks = element_blank(), 
  ...
  p2 <- ggplot(data =  p.sf) +
    ...
    scale_color_gradientn(colours = (viridis::viridis(100, direction = -1)), 
                          limits=c(expression_min, expression_max)) +
    labs(color = "Expression") + ggtitle(paste0("Gene: ", Keratin_pairs[l, 1]))+
    theme(axis.text = element_blank(), axis.ticks = element_blank(), 
...
Keratin_pairs_name <- paste0(Keratin_pairs[, 1], "_", Keratin_pairs[, 2])
LH_plots_n[[which(Keratin_pairs_name == "KRT17_KRT80")]]
LH_plots_n[[which(Keratin_pairs_name == "KRT17_KRT78")]]
LH_plots_n[[which(Keratin_pairs_name == "KRT17_KRT6B")]]
...
location_data_with_cluster <- as.data.frame(coords_data_cSCC)
