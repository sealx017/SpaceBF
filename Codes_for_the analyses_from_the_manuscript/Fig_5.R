...
# Delaunay network for running MERINGUE
  ...
  for(rho in c(-0.75, -0.5, -0.25, 0, 0.25, 0.5, 0.75)){
    ...
    for(iter in 1:10){
      ...
      z_all <- c(MASS::mvrnorm(1, rep(0, 2*N),
               kronecker(matrix(c(1, rho, rho, 1), nrow = 2), K))) # latent MVN distributed RV using Kronecker product structure
    ...
    saveRDS(fin_res, file = paste0(path,"Simulation_design_2_with_num_", 
...
system.time(MERINGUE_res <- parallel::parLapply(cl,  as.list(c(0.6, 1.8, 3.6, 7.2)), sim_runner))
...
for(num in num_all){
  for(rho in c(-0.75, -0.5, -0.25, 0, 0.25, 0.5, 0.75)){
    ...
    fin_res <- readRDS(file = paste0(path,"Simulation_design_2_with_num_", 
