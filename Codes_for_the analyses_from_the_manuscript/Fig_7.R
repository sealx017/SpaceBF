...
sim_runner<-function(iter){
  ...
  for(rho in c(-0.75, -0.5, -0.25, 0, 0.25, 0.5, 0.75)){
    ...
    for(r in c(1, 2)){
      ...
      # linear partitioning
      id1 <- coords[, 1] > median(coords[, 1]) & coords[, 2] <= median(coords[, 1])
      id2 <- coords[, 1] <= median(coords[, 1]) & coords[, 2] > median(coords[, 1])/r
      
      SD1 <- runif(length(which(id1 == T)), 0.3, 0.6)
      SD2 <- runif(length(which(id2 == T)), 0.3, 0.6)
      
      rho_vec[id1] <- rnorm(length(which(id1 == T)), mean = rho, sd = SD1)
      rho_vec[id2] <- rnorm(length(which(id2 == T)), mean = -rho, sd = SD2) 
    ...
    saveRDS(fin_res, file = paste0(path,"/Prior_comparison_MSE_Simulation_design_3_linear_iter_", iter, "_rho_", rho, ".rds")) 
  }
}
