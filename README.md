...
# library(Matrix); library(igraph);  library(sf); library(FNN);
...
<img src="README_files/figure-gfm/loading the coords-1.png" width="30%" />

``` r

# optional network to compute MERINGUE cross-correlation
w <- MERINGUE::getSpatialNeighbors(coords[, 1:2], filterDist = 3) # constructing Delauney network using  MERINGUE
...
<img src="README_files/figure-gfm/construct kernel matrix-1.png" width="30%" />
...
``` r
MM <- SpaceBF(y1, y2, X = NULL, G = mst.adjmat, which.model = "NB", nIter = 5000, verbose = F)  # decrease nIter for quicker results
# [1] "NB HS with MST will start with prior precision: 0.01, 0.01."
# [1] "500 / 5000"
# [1] "1000 / 5000"
# [1] "1500 / 5000"
# [1] "2000 / 5000"
...
# List of 5
#  $ Beta    : num [1:2500, 1:586] -1.94 -1.75 -1.87 -1.57 -1.87 ...
#  $ R       : num [1:2500, 1] 9.29 8.81 8.58 8.97 9.16 ...
#  $ Lambda  : num [1:2500, 1:584] 0.0223 0.0159 0.3278 0.6954 0.7098 ...
#  $ Tau     : num [1:2500, 1:2] 0.00354 0.00393 0.00339 0.00272 0.00234 ...
#  $ Deviance: num [1:2500, 1] 1181 1183 1167 1167 1190 ...
# NULL
```
...
# 2  0.3308921 0.08195055         0.8644156
# 3  0.3299198 0.07670019         0.7966709
# 4  0.3306464 0.11496412         0.6486339
# 5  0.2995344 0.09324329         3.2589386
```
...
(top row). The estimated slope is shown in a *z*-score type format,
i.e., scaled by mean and SD, in the bottom row. In the bottom right
sub-figure, locations where the local test produced a *p*-value \> 0.9
are greyed.
...
<img src="README_files/figure-gfm/p-value visualization-1.png" width="60%" />
...
*G* now denotes a mutual kNN network (with k = 3).

``` r

# construct a mutual 3-NN graph
kNN_obj <- kNN_construction(coords[, 1:2], k = 3)
w_knn <- as.matrix(kNN_obj[[1]]) # kNN adjacency matrix

# ICAR model fitting
MM_ICAR <- SpaceBF(y1, y2, X = NULL, G = w_knn, which.model = "NB", which.prior = "ICAR", nIter = 5000, verbose = F) 
# [1] "Your G is not an MST, the algorithm might take more run-time."
# [1] "NB ICAR will start with prior precision: 0.01, 0.01."
# [1] "500 / 5000"
# [1] "1000 / 5000"
# [1] "1500 / 5000"
# [1] "2000 / 5000"
...
<img src="README_files/figure-gfm/fit SpaceBF NB model with ICAR-1.png" width="60%" />
...
<img src="README_files/figure-gfm/compare HS vs. ICAR-1.png" width="60%" />

## Fit the spatial horseshoe model with a general graph (not MST)

We are fitting the NB model with the spatial horseshoe prior coupled
with the kNN network instead of the MST.

``` r
MM_HS_kNN <- SpaceBF(y1, y2, X = NULL, G = w_knn, which.model = "NB", which.prior = "HS", nIter = 5000, verbose = F) 
# [1] "Your G is not an MST, the algorithm might take more run-time."
# [1] "NB HS with general graph will start with prior precision: 0.01, 0.01."
# [1] "500 / 5000"
# [1] "1000 / 5000"
# [1] "1500 / 5000"
# [1] "2000 / 5000"
...
plot_estimates(MM_HS_kNN$Beta[, -c(1:N)], y1, y2, coords[, 1:2], which.model = "NB")
```

<img src="README_files/figure-gfm/fit SpaceBF NB model with horseshoe on kNN-1.png" width="60%" />

## Comparison between three models: HS models with MST and kNN, and ICAR model

We plot the results in a pairwise fashion.

``` r

L_results_HS_kNN <- local_res(MM_HS_kNN$Beta[, -c(1:N)])

plot_local_comparisons(L_results, L_results_HS_kNN)
```

<img src="README_files/figure-gfm/compare HS MST vs. HS Delaunay vs. ICAR-1.png" width="60%" />

``` r
plot_local_comparisons(L_results_ICAR, L_results_HS_kNN)
```

<img src="README_files/figure-gfm/compare HS MST vs. HS Delaunay vs. ICAR-2.png" width="60%" />
